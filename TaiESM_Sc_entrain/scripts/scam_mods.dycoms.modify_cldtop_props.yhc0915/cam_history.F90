...
!-----------------------------------------------------------------------
     ...
     do i=1,max(ngroup(t),1)
        ...
        if(interpolate) then
           ...
           !<--- yhc 2023-11-24 
           !     The values of these fields have invalid values (e.g. Infinity).
           !     Reset the values to zero to avoid pio_write_darray shutdown
           !     with the following error messages:
           !        "NetCDF: Numeric conversion not representable
           !         pio_support::pio_die:: myrank=          -1 : ERROR: 
           !         pionfwrite_mod::write_nfdarray_double:         241 :"
           if (     tape(t)%hlist(f)%field%name .eq. "HACLDICE"   &
               .or. tape(t)%hlist(f)%field%name .eq. "HACLDLIQ"   &
               .or. tape(t)%hlist(f)%field%name .eq. "HAQ"        &
               .or. tape(t)%hlist(f)%field%name .eq. "VACLDICE"   &
               .or. tape(t)%hlist(f)%field%name .eq. "VACLDLIQ"   &
               .or. tape(t)%hlist(f)%field%name .eq. "VAQ"        &
              ) then
             tape(t)%hlist(f)%hbuf = 0.
             !write(iulog,*) 'WARNING: dump_field, the values of this variable
             !is reset to zeros to avoid [pio_write_darray] error,
             !',trim(tape(t)%hlist(f)%field%name)
             write(iulog,*) 'WARNING: dump_field, the values of ['//trim(tape(t)%hlist(f)%field%name)//'] are reset to zeros to avoid [pio_write_darray] error'
           end if
           !---> yhc 2023-11-24
