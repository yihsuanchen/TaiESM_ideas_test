...
subroutine rad_cnst_get_mode_props(list_idx, mode_idx, &
   ...
   rhcrystal, rhdeliques , sgnum, sigmaglo,sigmaghi)   !add sgnum by ICTsai
...
!add sgnum by ICTsai
   real(r8),  optional, intent(out) :: sgnum
   real(r8),  optional, intent(out) :: sigmaglo
   real(r8),  optional, intent(out) :: sigmaghi
!ICTsai
...
!add sgnum by ICTsai
   if (present(sgnum))        call physprop_get(id, sgnum=sgnum)
   if (present(sigmaglo))     call physprop_get(id, sigmaglo=sigmaglo)
   if (present(sigmaghi))     call physprop_get(id, sigmaghi=sigmaghi)
!ICTsai
