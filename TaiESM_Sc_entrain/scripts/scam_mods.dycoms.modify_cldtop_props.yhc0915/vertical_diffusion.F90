module vertical_diffusion
  ...
  ! -------------------
  ! ycw Nov 2014
  !  port the code to CAM5.3
  !  This is actually the interface for all three cases.
  !  Now the HBGFS is added as a new case in this interface.
  !  speciall attention to subroutine compute_vdiff! 
  !  HBGFS has its own subroutine compute_hbgfs_diff
  ! --------------------
  !joanne revised on 2013/01/21
  !joanne chen Nov 2012 modified to accommadte H&P 2011 PBL scheme
  !the program structure is based on the structure of HB scheme,
  !the evaluation of PBL height, diffusivity of local and non-local scheme are also
  !the same as HB scheme, only plus cloud-top-driven diffusivity into local and
  !non-local diffusivity.
  !  In this module, I add variables for diagnosing diffusivities. Since the diffusivity includes forcing both
  !from surface and cloud top, I modified every diffusivity term associated with counter gradients as only
  !surface diffusivite dkux,dktx.
  !module eddy_diff and diffusion_solver also need to be modified slightly.
  !-------------------------------
...
contains
  ...
  subroutine vertical_diffusion_init(pbuf2d)
    ...
    ! +++ Yi-Chi
    use hbgfs_diff,           only : init_hbgfs_diff
    use hbgfs_diffusion_solver, only : init_hbgfs_vdiff
    ! --- Yi-Chi
   ...
   if (masterproc) write(iulog, fmt='(a,i3,5x,a,i3)') 'NTOP_EDDY  =', ntop_eddy, 'NBOT_EDDY  =', nbot_eddy
    ...
    ! +++ Yi-Chi : add case 'HBGFS'
    case ( 'HBGFS')
        if( masterproc ) write(iulog,*) 'vertical_diffusion_init: eddy_diffusivity scheme: GFS cloud-top + Holtslag and Boville'
        call init_hbgfs_diff(gravit, cpair, ntop_eddy, nbot_eddy, pref_mid, &
                          karman, eddy_scheme)
        call addfld('HBGFS_ri', 'no',      pver,  'A',  'Richardson Number (HB Scheme), I',  phys_decomp )
    ! --- Yi-Chi   
    end select
    ...
    select case ( eddy_scheme )
    case ( 'HBGFS' )
    call init_hbgfs_vdiff( r8, rair, gravit, do_iss, errstring )
    call handle_errmsg(errstring, subname="init_hbgfs_vdiff")
    case default
...
!joanne add variables
    call addfld( 'CGH'         , 'J/kg/m'   , pverp  , 'A', 'Counter-gradient term for heat', phys_decomp )
    call addfld( 'KBFS'        , 'm2/s3'    , 1      , 'A', 'sfc kinematic buoyancy flux ', phys_decomp)
    call addfld( 'KHFS'        , 'mK/s'     , 1      , 'A', 'surface kinematic heat flux', phys_decomp)
    call addfld( 'KQFS'        , 'm/s'      , 1      , 'A', 'sfc kinematic constituent flux', phys_decomp)
    call addfld( 'KVF'         , 'm2/s'     , pverp  , 'A', 'Vertical diffusion diffusivities (heat/moisture)'  , phys_decomp )
    !call addfld( 'KSFT'         , 'm2/s'    , pverp  , 'A', 'Vertical diffusion diffusivities (heat/moisture)'  , phys_decomp )
    !call addfld( 'KSFU'         , 'm2/s'    , pverp  , 'A', 'Vertical diffusion diffusivities (heat/moisture)'  , phys_decomp )
    call addfld( 'KSCT'         , 'm2/s'    , pverp  , 'A', 'Cloud-top driven vertical diffusion diffusivities (heat/moisture)'  , phys_decomp )
    call addfld( 'KSCU'         , 'm2/s'    , pverp  , 'A', 'Cloud-top driven Vertical diffusion diffusivities (ommentum)'  , phys_decomp )
    call addfld( 'THETA'         , 'K'      , pver  , 'A', 'Potential temperature seen by PBL'  , phys_decomp )
!joanne
...
!!!joanne add variables
        ...
        call add_default( 'CGS     ', 1, ' ' )
        call add_default( 'CGH     ', 1, ' ' )
        call add_default( 'TPERT     ', 1, ' ' )
        call add_default( 'QPERT     ', 1, ' ' )
        call add_default( 'USTAR   ', 1, ' ' )
        call add_default( 'KBFS     ', 1, ' ' )
        call add_default( 'KHFS     ', 1, ' ' )
        call add_default( 'KQFS     ', 1, ' ' )
        call add_default( 'KVF     ', 1, ' ' )
        !call add_default( 'KSFT     ', 1, ' ' )
        !call add_default( 'KSFU     ', 1, ' ' )
        call add_default( 'KSCT     ', 1, ' ' )
        call add_default( 'KSCU     ', 1, ' ' )
...
!!!joanne
  ...
  subroutine vertical_diffusion_tend( &
    ...
    ! Yi-Chi : use individual scripts
    use hbgfs_diff,            only : compute_hbgfs_diff
    use hbgfs_diffusion_solver,only : hbgfs_compute_vdiff
    ! Yi-Chi end
    ...
    !joanne
    real(r8), pointer, dimension(:,:) :: qrs                        ! SW radiative cooling rate
    !joanne
    ...
    ! +++ Yi-Chi : variable declaration for GFSHB
    real(r8) :: swh(pcols,pverp)                                 ! short wave heating
    real(r8) :: hlw(pcols,pverp) 
    ...
    real(r8) :: kvf(pcols,pverp)                     ! check GFS diff.
    real(r8) :: ksct(pcols,pverp)                     ! check GFS diff.
    real(r8) :: kscu(pcols,pverp)                     ! check GFS diff.
    !real(r8) :: ksft(pcols,pverp)                     ! check GFS diff.
    !real(r8) :: ksfu(pcols,pverp)                     ! check GFS diff.

    real(r8) :: dkux(pcols,pverp)               ! GFS
    real(r8) :: dktx(pcols,pverp)               ! GFS
    ! --- Yi-Chi
    ...
    !<--- yhc0915, modify s and qv right above the Sc top
    logical, parameter :: do_modify_cldtop_props = .true. 
...
!      call outfld( 'TPERT'        , tpert,                     pcols, lchnk )
       ...
       call outfld( 'KBFS'          , kbfs,                       pcols, lchnk )
       call outfld( 'KHFS'          , khfs,                       pcols, lchnk )
       call outfld( 'KQFS'          , kqfs,                       pcols, lchnk )
       call outfld( 'KVF'          , kvf,                       pcols, lchnk )
       !call outfld( 'KSFT'          , ksft,                       pcols, lchnk )
       !call outfld( 'KSFU'          , ksfu,                       pcols, lchnk )
       call outfld( 'KSCT'          , ksct,                       pcols, lchnk )
       call outfld( 'KSCU'          , kscu,                       pcols, lchnk )
       call outfld( 'THETA'      , th,                       pcols, lchnk )
       !!!joanne
    ...
    case ( 'HBGFS' )
     ...
     !                consistency with the previous HB scheme.
       ...
       th(:ncol,:pver) = state%t(:ncol,:pver) * state%exner(:ncol,:pver)
!joanne include shortwave and longwave heating rate to calculate cloud-top cooling
!and also add variables in compute_hb_diff
!!     swh(:ncol,:pver) = pbuf(qrs_idx)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
!!     hlw(:ncol,:pver) = pbuf(qrl_idx)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
!!     swh(:ncol,:pver) = swh(:ncol,:pver)/(state%pdel(:ncol,:pver)*cpair)
!!     hlw(:ncol,:pver) = hlw(:ncol,:pver)/(state%pdel(:ncol,:pver)*cpair)
       ! +++ Yi-Chi : change to new way of physical buffer
       call pbuf_get_field(pbuf, qrl_idx,   qrl   )
       ...
       ! --- Yi-Chi
       swh(:ncol,:pver) = qrs(:,:)/(state%pdel(:ncol,:pver)*cpair)
       hlw(:ncol,:pver) = qrl(:,:)/(state%pdel(:ncol,:pver)*cpair)
       ! --- Yi-Chi
!joanne
       call compute_hbgfs_diff( lchnk     , ncol    ,   state%pint,                          &
                             th        , state%t , state%q , state%zm , state%zi, &
                             state%pmid, state%u , state%v , tautotx  , tautoty , &
...
!!!joanne
       ...
       call outfld( 'HBGFS_ri',          ri,         pcols,   lchnk )
       wpert = 0._r8
    ...
    !<--- yhc0915, modify s and qv right above the Sc top
    !              mass and energy is not conserved if returing the tendencies with modified s and qv
    if (do_modify_cldtop_props) then
      ...
      write(iulog, *) 'yhc0915, s_tmp',s_tmp
      write(iulog, *) 'yhc0915, qv',q_tmp(:,:,1)

      ! modify s_tmp and qv right above the cloud top
      do i = 1, ncol
        k = k_cldtop(i)
        if (k > 1) then
          s_tmp(i,k-1)   = ratio_qq*s_tmp(i,k-1)   + (1._r8-ratio_qq)*s_tmp(i,k)
          q_tmp(i,k-1,1) = ratio_qq*q_tmp(i,k-1,1) + (1._r8-ratio_qq)*q_tmp(i,k,1)
        endif
      enddo

      write(iulog, *) 'yhc0915, k_cldtop',k_cldtop      
      write(iulog, *) 'yhc0915, new s_tmp',s_tmp
      write(iulog, *) 'yhc0915, new qv',q_tmp(:,:,1)
      write(iulog, *) 'yhc0915, cldliq',q_tmp(:,:,ixcldliq)
      write(iulog, *) 'yhc0915, kvh', kvh
    endif ! end if of do_modify_cldtop_props
    !---> yhc0915, 
    ...
    if( any(fieldlist_wet) ) then
        ! +++ Yi-Chi : add HBGFS as a case
        select case (eddy_scheme)
        case ( 'HBGFS')
        call hbgfs_compute_vdiff( state%lchnk           ,                               &
                            ...
                            do_molec_diff   , dktx, dkux , compute_molec_diff , vd_lu_qdecomp,kvt)
        call handle_errmsg(errstring, subname="hbgfs_compute_vdiff", &
    ...
    if( any( fieldlist_dry ) ) then
        ...
        ! +++ Yi-Chi : add HBGFS case
        select case (eddy_scheme)
        case ( 'HBGFS')

        call hbgfs_compute_vdiff( lchnk           ,                               &
                            ...
                            tautmsx         , tautmsy            , dtk           , topflx       , errstring   , &
                            tauresx         , tauresy            , 1             , cpairv(:,:,state%lchnk), rairi       , &
                            do_molec_diff   , dktx, dkux , compute_molec_diff , vd_lu_qdecomp)
        call handle_errmsg(errstring, subname="hbgfs_compute_vdiff", &
...
!!
!!joanne modified the diffusivity associated with counter gradient
          slflx(i,k) = kvh(i,k) * &
                          ( - rhoair*(sl(i,k-1)-sl(i,k))/(state%zm(i,k-1)-state%zm(i,k)))&
                       + dktx(i,k)*cgh(i,k)
          qtflx(i,k) = kvh(i,k) * &
                          ( - rhoair*(qt(i,k-1)-qt(i,k))/(state%zm(i,k-1)-state%zm(i,k)))&
                       + rhoair*(cflx(i,1)+cflx(i,2)+cflx(i,3))*cgs(i,k)*dktx(i,k)
!!cam_ori slflx(i,k) = kvh(i,k) * &
!!                        ( - rhoair*(sl(i,k-1)-sl(i,k))/(state%zm(i,k-1)-state%zm(i,k)) &
!!                          + cgh(i,k) )
!!        qtflx(i,k) = kvh(i,k) * &
!!                        ( - rhoair*(qt(i,k-1)-qt(i,k))/(state%zm(i,k-1)-state%zm(i,k)) &
...
!joanne modified the diffusivity associated with counter gradient
!cam_ori  slflx_cg(i,k) = kvh(i,k) * cgh(i,k)
!         qtflx_cg(i,k) = kvh(i,k) * rhoair * ( cflx(i,1) + cflx(i,2) + cflx(i,3) ) * cgs(i,k)
          slflx_cg(i,k) = dktx(i,k) * cgh(i,k)
          qtflx_cg(i,k) = dktx(i,k) * rhoair * ( cflx(i,1) + cflx(i,ixcldliq) + cflx(i,ixcldice) ) * cgs(i,k)
!!joanne
          uflx_cg(i,k)  = 0._r8
          vflx_cg(i,k)  = 0._r8
       end do
