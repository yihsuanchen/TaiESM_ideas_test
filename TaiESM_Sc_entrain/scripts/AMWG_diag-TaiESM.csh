...
module purge
module load aws-sdk-cpp_gcc-10.3.1/1.11.30 netcdf_hdf5-1.10.5_gcc-10.3.1/4.9.1
module load nco_netcdf-4.9.1_gcc-10.3.1/ ncl_gcc-4.8.5/
#module load rcec/tools-intel19 rcec/ncl/6.6.2-gcc 
...
set test_casename = q1-taiesm1_modified_Ke_Sc_top_sl.F_2000_TAI.f09_f09.0330_1627
set test_filetype = monthly_history 
#set test_filetype = time_series

set test_path         =  /lfs/home/yihsuanc/test/j1-modified_Ke_Sc_top/
set test_path_history =  ${test_path}/${test_casename}/atm/hist/
set test_path_climo   =  ${PWD}/diag_archive/climo/${test_casename}/ 
set test_path_diag    =  ${PWD}/diag_archive/diag/${test_casename}/
...
set cntl_casename = q1-taiesm1_CTL.F_2000_TAI.f09_f09.0330_1612
set cntl_filetype = monthly_history 

#set cntl_filetype = time_series

set cntl_path_history = ${test_path}/${cntl_casename}/atm/hist/
set cntl_path_climo   = ${PWD}/diag_archive/climo/${cntl_casename}/  
...
# Number of years is: $test_nyrs         (must be >= 1)
 ...
 set test_first_yr =   2 
 set test_nyrs     =   5 
 set iStrt = $test_first_yr
 set iLast = `expr $test_first_yr + $test_nyrs - 1`

# FOR CNTL == USER ONLY (otherwise skip this section)
# First year of data is: $cntl_first_yr     (must be >= 1)
# Number of years is: $cntl_nyrs         (must be >= 1)

 set cntl_first_yr =  ${test_first_yr} 
 set cntl_nyrs     =  ${test_nyrs}
...
set set_16 = 0    # (0=ON,1=OFF)  Budget Terms for Select stations
...
set density   = 300  # pixels/inch, use larger number for higher 
...
# LCCR machines (BIG)
  setenv DIAG_HOME /lfs/home/lama/work/lccr_diag1.0/amwg_diag/ 
...
#JOY
#set cntl_rootname  = 'f09.B1850CN.cam53.cam.h0.'
#
    echo ' '
...
if ($web_pages == 0) then
  ...
  if ($CNTL == OBS) then
    setenv WEBDIR ${test_path_diag}$test_casename-obs_${iStrt}-${iLast}
    if (! -e $WEBDIR) mkdir $WEBDIR
    cd $WEBDIR
    $HTML_HOME/setup_obs $test_casename $image
    cd $test_path_diag
    set tarfile = ${test_casename}-obs_${iStrt}-${iLast}.tar
  else          # model-to-model 
    setenv WEBDIR ${test_path_diag}${test_casename}-${cntl_casename}_${iStrt}-${iLast}
    if (! -e $WEBDIR) mkdir $WEBDIR
    cd $WEBDIR
    $HTML_HOME/setup_2models $test_casename $cntl_casename $image
    cd $test_path_diag
    set tarfile = $test_casename-${cntl_casename}_${iStrt}-${iLast}.tar
  endif
endif
...
if ($web_pages == 0) then
  ...
  if ($CNTL == OBS) then
    setenv WEBDIR ${WKDIR}$test_casename-obs_${iStrt}-${iLast}
    if (! -e $WEBDIR) mkdir $WEBDIR
    cd $WEBDIR
    $HTML_HOME/setup_obs $test_casename $image
    cd $WKDIR
    ...
    setenv WEBDIR ${WKDIR}$test_casename-${cntl_casename}_${iStrt}-${iLast}
    if (! -e $WEBDIR) mkdir $WEBDIR
    cd $WEBDIR
    $HTML_HOME/setup_2models $test_casename $cntl_casename $image 
    cd $WKDIR
